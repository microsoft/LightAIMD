...
i64 check_forces(f64 *forces, u64 cc_method_id)
{
    ...
    */
    ...
    for (u64 i = 0; i < 9; ++i)
    {
        f64 ref_force = self_ref_forces_on_nuclei[cc_method_idx][i];
    ...
    printf("%s: max force difference is |%.12e - %.12e| = %.6e (Ref).\n", CC_METHODS[cc_method_idx], max_delta_force, max_delta_ref_force, max_delta);

    max_delta = 0.0;
    max_delta_force = 0.0;
    max_delta_ref_force = 0.0;
    for (u64 i = 0; i < 9; ++i)
    {
    ...
    printf("%s: max force difference is |%.12e - %.12e| = %.6e (PySCF, lvl 5).\n", CC_METHODS[cc_method_idx], max_delta_force, max_delta_ref_force, max_delta);
