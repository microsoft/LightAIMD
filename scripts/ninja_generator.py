...
def generate_ninja_script(debug=False):
    ...
    def ninja_compile_cuda_objects(writer):
        for target, (input_file, flags) in cuda_object_targets.items():
            writer.write(f"build {os.path.join(config['obj_dir'], target)}: nvcc_compile {os.path.join(config['src_dir'], input_file)}\n")
            if flags != '':
    ...
    def ninja_make_executables(writer):
        for target, (files, flags) in executable_targets.items():
            ...
            writer.write(f"build {os.path.join(config['bin_dir'], target)}: make_executable {' '.join(input_files)}\n")
    ...
    write_ninja_file(os.path.join(config['active_build_dir'], 'build.ninja'), ['c', 'cpp'])
